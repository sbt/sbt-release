...
object ReleaseStateTransformations {
  ...
  private[sbtrelease] lazy val initialVcsChecks = { st: State =>
    ...
    if ( hasUntrackedFiles && !extracted.get( ignoreUntrackedFiles ) )
    {
        sys.error( "Aborting release: untracked files. Remove them or specify 'ignoreUntrackedFiles := true' in settings" )
    }
